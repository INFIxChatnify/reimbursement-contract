...
/**
 * @title EnhancedProjectReimbursement
 ...
 */
contract EnhancedProjectReimbursement is 
    Initializable,
    ...
    PausableUpgradeable
{
    ...
    function commitApproval(uint256 requestId, bytes32 commitment) external whenNotPaused notEmergencyStopped nonReentrant {
        ...
        if (request.status == Status.FinanceApproved && request.committeeAdditionalApprover == address(0) && !hasRole(COMMITTEE_ROLE, msg.sender)) revert UnauthorizedApprover();
        if (request.status == Status.FinanceApproved && request.committeeAdditionalApprover != address(0) && !hasRole(DIRECTOR_ROLE, msg.sender)) revert UnauthorizedApprover();
